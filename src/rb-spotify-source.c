...
void
rbspotifysource_search(RBSource *source, RBSourceSearch *search,
		const char *cur_text, const char *new_text);

void
rbspotify_search_complete_cb(sp_search *result, void *userdata);
...
static void entry_set_string_prop(RhythmDB *db, RhythmDBEntry *entry,
...
void rbspotify_search_complete_cb(sp_search *result, void *userdata)
{
	...
	g_debug( "Result callback\n");
	if (!result)
		return;

	g_debug("Search object in callback: %p\n", result);
	...
	if (result && SP_ERROR_OK == sp_search_error(result))
	{
		...
		for (i = 0; i < sp_search_num_tracks(result) && i < 200; ++i)
		{
			...
			entry_set_string_prop(self->priv->db, entry, RHYTHMDB_PROP_TITLE,
					sp_track_name(track));

			/* album */
			entry_set_string_prop(self->priv->db, entry, RHYTHMDB_PROP_ALBUM,
					sp_album_name(sp_track_album(track)));

			/* artist */
			entry_set_string_prop(self->priv->db, entry, RHYTHMDB_PROP_ARTIST,
			...
			rhythmdb_entry_set(self->priv->db, entry, RHYTHMDB_PROP_DURATION,
					&value);
			g_value_unset(&value);
		}
		rhythmdb_commit(self->priv->db);
...
void rbspotifysource_search(RBSource *source, RBSourceSearch *search,
		const char *cur_text, const char *new_text)
{
	g_debug("In rbspotifysource_search()");
	printthreadname();
	if (new_text != NULL && g_utf8_strlen(new_text, 10) > 2)
	{
		g_debug( "*Searching!* Current: %s // New: %s\n", cur_text,
				new_text);
		...
		g_debug("search session: %p\n", self->priv->sess);
		sp_search *s = sp_search_create(self->priv->sess, g_strdup(new_text), 0,
				100, 0, 100, 0, 100, 0, 100, SP_SEARCH_STANDARD,
				rbspotify_search_complete_cb, self);
		g_debug("Search object: %p\n", s);
